...
// pcb queues
pcb_queue_t
    priority_queue_array[NUM_PRIORITY_QUEUES];  // queue of running/runnable
                                                // threads
...
void cancel_and_join_pcb(pcb_t* pcb_ptr) {
  ...
  // spthread_suspend() will cause the target thread to call sigsuspend, which
  // is a cancellation point forces the spthread to hit a cancellation point
...
void pennos_kernel(void) {
  ...
  all_unreaped_pcb_vector =
      pcb_vec_new(0, pcb_destroy);  // need to be destroyed later
  ...
  if (pcb_init(thrd_init, &temp_pcb_ptr, NULL, QUEUE_PRIORITY_0, pid_count++,
               INIT_THREAD_NAME) == -1) {
...
void* thrd_init_fn([[maybe_unused]] void* arg) {
  ...
  if (pcb_init(thrd_shell, &temp_pcb_ptr, parent_pcb_ptr, QUEUE_PRIORITY_0,
  ...
  self_pcb_ptr->status = THRD_BLOCKED;  // modify self status to BLOCKED as it
                                        // will block and wait for shell to join
  spthread_join(thrd_shell, NULL);      // wait for shell thread to join
  // init thread block wait to for shell thread to join
  // ...
  // till shell user exit by ctrl-D
  self_pcb_ptr->status =
      THRD_RUNNING;  // modify self status to RUNNING as shell thread has joined
...
void lifecycle_event_log(pcb_t* proc, const char* event, void* info) {
  ...
  klog("[%5d]\t%s\t%d\t%d\tprocess", global_clock, event, thrd_pid(proc),
       thrd_priority(proc));
}
