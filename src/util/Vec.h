...
/* Sets the specified element of the Vec to the specified value on index, regardless
 * of current length
 * 
 * @param self    a pointer to the vector who's element we want to set.
 * @param index   the index of the element to set. If it is over current length,
 * the capacity will be ensured and elements before index and after previous final element
 ...
 * @note The part expanded after resize is not initialized.
 */
void vec_resize(Vec* self, size_t new_capacity);

/* Resizes the container to a new specified capacity, and initialzed the 
 * expanded part to null.
 * Does nothing if new_capacity <= self->length
 *
 * @param self         a pointer to the vector we want to resize.
 * @param new_capacity the new capacity of the vector.
 ...
 * reallocation are invalidated.
 * @post The removed elements are destructed (cleaned up).
 * @note The part expanded after resize is initialized to null.
 */
 ...
 * @pre Assumes self points to a valid vector. If it is null,
